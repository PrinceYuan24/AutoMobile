...
class ROSPlanner:
    def __init__(self, heuristic_func, weight_func, num_vertices, connection_radius,
        ...
        if os.path.exists(graph_file):
            ...
            # self.planning_env.visualize_graph(self.G)
        else:
	        #wzy
    ...
    def plan_to_goal(self, start, goal):
        ...
        print('path length', lazy_astar.path_length(G, path))
    ...
    def plan_multi_goals(self, start):
        ...
        for i in range(len(self.goals) - 1):
            if end < len(self.goals):
                temp_path = self.plan_to_goal(self.goals[start], self.goals[end])
                if temp_path == False:
                    end = end + 1
                    continue
...
if __name__ == '__main__':
    ...
    ROSPlanner(heuristic, weight, num_vertices=400, connection_radius=250)
