...
if __name__ == "__main__":
    ...
    G, start_id = graph_maker.add_node(G, args.start, env=planning_env,
        connection_radius=args.connection_radius, start_from_config=True)
    G, goal_id = graph_maker.add_node(G, args.goal, env=planning_env,
        connection_radius=args.connection_radius, start_from_config=False)

    # Uncomment this to visualize the graph
    planning_env.visualize_graph(G)

    print('time construct ', time.time() - start)
    plantime = time.time()
    try:
        ...
        if args.lazy:
            weight = lambda n1, n2: planning_env.edge_validity_checker(
                G.nodes[n1]['config'], G.nodes[n2]['config'])
            path = lazy_astar.astar_path(G,planning_env,
        ...
        print('time planning: ', time.time() - plantime)
        print('path length', astar.path_length(G, path))
        #print('edges', astar..edge_count)
