...
class ParticleFilter():
  ...
  def clicked_pose_cb(self, msg):
    ...
    #YOUR CODE HERE
    self.particles[:,0]=np.random.normal(pose.position.x, 0.5, self.N_PARTICLES)
    self.particles[:,1]=np.random.normal(pose.position.y, 0.5, self.N_PARTICLES)
    angle = Utils.quaternion_to_angle(pose.orientation)
    self.particles[:,2]=np.random.normal(angle, 0.5, self.N_PARTICLES)
