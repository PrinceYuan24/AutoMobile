...
#KM_V_NOISE = 0.05
# KM_DELTA_NOISE = 0.5 # Kinematic car delta noise std dev
KM_DELTA_NOISE = 0.1
...
class KinematicMotionModel:
  ...
  def apply_motion_model(self, proposal_dist, control):
    ...
    #YOUR CODE HERE
    # temp_proposal_dist = copy.deepcopy(proposal_dist)
    temp_proposal_dist=np.zeros((proposal_dist.shape[0],3), dtype=np.float32)
    temp_proposal_dist[:,:] = proposal_dist[:,:]
    v_dist = np.random.normal(v_mag,KM_V_NOISE,proposal_dist.shape[0])
    delta_dist = np.random.normal(delta_mag,KM_DELTA_NOISE,proposal_dist.shape[0])

    # proposal_dist[:,2]= temp_proposal_dist[:,2] + np.multiply((v_dist/self.CAR_LENGTH),np.tan(delta_dist))*dt
    # proposal_dist[:,0] = temp_proposal_dist[:,0] + np.multiply(np.true_divide(self.CAR_LENGTH,np.tan(delta_dist)),(np.sin(proposal_dist[:,2]) - np.sin(temp_proposal_dist[:,2])))
    # proposal_dist[:,1] = temp_proposal_dist[:,1] + np.multiply(np.true_divide(self.CAR_LENGTH,np.tan(delta_dist)),(-np.cos(proposal_dist[:,2]) + np.cos(temp_proposal_dist[:,2])))
    
    proposal_dist[:,2]= temp_proposal_dist[:,2] + (v_dist/self.CAR_LENGTH)*np.tan(delta_dist)*dt
    proposal_dist[:,0] = temp_proposal_dist[:,0] + (self.CAR_LENGTH/np.tan(delta_dist)*(np.sin(proposal_dist[:,2]) - np.sin(temp_proposal_dist[:,2])))
    proposal_dist[:,1] = temp_proposal_dist[:,1] + (self.CAR_LENGTH/np.tan(delta_dist)*(-np.cos(proposal_dist[:,2]) + np.cos(temp_proposal_dist[:,2])))
    ...
    proposal_dist[:,2] = np.random.normal(proposal_dist[:,2],KM_THETA_FIX_NOISE)
