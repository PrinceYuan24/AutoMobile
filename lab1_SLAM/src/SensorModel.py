...
class SensorModel:
  ...
  def lidar_cb(self, msg):
    ...
    #print 'Finished lidar cb'sensor_model_table
  ...
  '''
    ...
    Element (r,d) ofp_rand the table is the probability of observing measurement r
  ...
  def precompute_sensor_model(self, max_range_px):
    ...
    for zk_str in range(0, max_range_px + 1):
      p_short = np.zeros(max_range_px + 1)
      p_hit = np.zeros(max_range_px + 1)
      p_max = np.zeros(max_range_px + 1)
      p_rand = 1.0 / (max_range_px + 1) * np.ones(max_range_px + 1)
      p_rand[max_range_px] = 0

      for zk in range(max_range_px+1):
        if zk < zk_str:
          p_short[zk] = 2*(float(zk_str) - float(zk))/float(zk_str)/float(zk_str)

        p_hit[zk] = (1/np.sqrt(2*np.pi*(SIGMA_HIT**2)))*np.exp(-(np.power((zk-zk_str),2)/(2*(SIGMA_HIT**2))))

      p_max[max_range_px] = 1
      zt_matrix = np.asarray([Z_HIT,Z_SHORT,Z_MAX,Z_RAND])
      p_matrix = np.asarray([p_hit,p_short,p_max,p_rand])

      p_zk = (np.dot(zt_matrix,p_matrix)).T
      sump = np.sum(p_zk)
      p_zk = p_zk / sump

      sensor_model_table[:,zk_str] = p_zk[:]
