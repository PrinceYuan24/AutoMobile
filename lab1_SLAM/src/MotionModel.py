...
#Tune these Values!
# KM_V_NOISE = 0.06 # Kinematic car velocity noise std dev
...
# KM_DELTA_NOISE = 0.4 # Kinematic car delta noise std dev
...
class KinematicMotionModel:
  ...
  def apply_motion_model(self, proposal_dist, control):
    ...
    #YOUR CODE HERE
    proposal_old=np.zeros((proposal_dist.shape[0],3), dtype=np.float32)
    proposal_old[:,:] = proposal_dist[:,:]
    
    size = proposal_dist.shape[0]
    pro_v = np.random.normal(v_mag, KM_V_NOISE, size)
    pro_delta = np.random.normal(delta_mag, KM_DELTA_NOISE, size)

    proposal_dist[:,2] = proposal_old[:,2] + (pro_v/self.CAR_LENGTH)*np.tan(pro_delta)*dt
    proposal_dist[:,0] = proposal_old[:,0] + (self.CAR_LENGTH/np.tan(pro_delta)*(np.sin(proposal_dist[:,2]) - np.sin(proposal_old[:,2])))
    proposal_dist[:,1] = proposal_old[:,1] + (self.CAR_LENGTH/np.tan(pro_delta)*(-np.cos(proposal_dist[:,2]) + np.cos(proposal_old[:,2])))
